...
class m191207_170408_group extends Migration
{
    ...
    public function safeUp()
    {
        $this->createTable('group', [
            ...
            'flight_id' => $this->integer()->null(),
            'count_people' => $this->integer(),
            'created_at' => $this->timestamp()->defaultExpression('CURRENT_TIMESTAMP'),
            'updated_at' => $this->timestamp()->defaultValue(null)->append('ON UPDATE CURRENT_TIMESTAMP'),
            'user_id' => $this->integer()->null()->comment('Created by'),
        ]);

        $this->createIndex('idx-group-region_id', 'group', 'region_id');
        $this->addForeignKey('fk-group-region_id', 'group', 'region_id', 'region', 'id', 'RESTRICT');

        $this->createIndex('idx-group-flight_id', 'group', 'flight_id');
        $this->addForeignKey('fk-group-flight_id', 'group', 'flight_id', 'flight', 'id', 'SET NULL');

        $this->createIndex('idx-group-user_id', 'group', 'user_id');
        $this->addForeignKey('fk-group-user_id', 'group', 'user_id', 'user', 'id', 'SET NULL');
    }
